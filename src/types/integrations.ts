...
/**
 * OpenClaw Integration - Per-user AI agent connection
 */
export interface OpenClawIntegration {
  ...
  // Connection settings
  enabled: boolean;
  webhookUrl: string;          // User's OpenClaw webhook endpoint
  apiKey: string;              // Generated by Moltboard, used by their OpenClaw
  ...
  // Stats
  messagesReceived: number;    // From user via OpenClaw
  messagesSent: number;        // Responses back to OpenClaw
  lastMessageAt?: Date;
...
/**
 * OpenClaw webhook payload (outbound to user's OpenClaw)
 */
export interface OpenClawOutboundPayload {
...
/**
 * OpenClaw webhook payload (inbound from user's OpenClaw)
 */
export interface OpenClawInboundPayload {
  type: 'response';
  message: {
    content: string;
    replyTo?: string;
