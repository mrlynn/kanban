...
/**
 * Clawdbot Integration - Per-user AI agent connection
 */
export interface ClawdbotIntegration {
  _id?: ObjectId;
  id: string;
  tenantId: string;
  userId: string;              // Owner of this integration
  
  // Connection settings
  enabled: boolean;
  webhookUrl: string;          // User's Clawdbot webhook endpoint
  apiKey: string;              // Generated by Moltboard, used by their Clawdbot
  ...
  // Stats
  messagesReceived: number;    // From user via Clawdbot
  messagesSent: number;        // Responses back to Clawdbot
  lastMessageAt?: Date;
...
/**
 * Clawdbot webhook payload (outbound to user's Clawdbot)
 */
export interface ClawdbotOutboundPayload {
...
/**
 * Clawdbot webhook payload (inbound from user's Clawdbot)
 */
export interface ClawdbotInboundPayload {
  type: 'response';
  message: {
    content: string;
    replyTo?: string;
